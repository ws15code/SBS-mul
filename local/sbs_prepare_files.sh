...
for x in train eval; do
    ...
    case "$LCODE" in
        AR)
            /export/ws15-pt-data/rsloan/ar_to_ipa.sh --utts $tmpdir/downsample/${x}_basenames_wav --transdir "$TRANSDIR/${full_name}" | sed 's/SIL//g' | sed 's/   */ /g' | sed 's/^ *//g' | sed 's/ *$//g' > $tmpdir/${LCODE}_${x}.trans 
            ;;
        DT)
            ;;
        MD)
